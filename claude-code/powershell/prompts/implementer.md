...
- **`ralph-get-next-feature`** - Find the next feature to work on (script: `.claude/skills/ralph/get-next-feature/get-next-feature.ps1`)
...
> **REMINDER:** You must implement exactly ONE feature this iteration, then exit. Do NOT batch multiple features. Use the companion `.ps1` scripts for ALL `feature_list.json` writes — never edit it directly.
...
# Script: ./.claude/skills/ralph/get-feature-stats/get-feature-stats.ps1
...
# Script: ./.claude/skills/ralph/get-next-feature/get-next-feature.ps1
...
- ✅ **Use companion `.ps1` scripts for ALL feature_list.json writes** (they recalculate stats atomically)
...
### DON'T:
- ❌ **NEVER edit feature_list.json directly** — always use companion `.ps1` scripts (direct manipulation causes stats drift and data corruption)
