...
- **`ralph-get-next-feature`** - Find the next feature to work on (script: `ralph skill get-next-feature`)
- **`ralph-update-feature-status`** - Change feature status (script: `ralph skill update-feature-status`)
- **`ralph-increment-feature-attempts`** - Track failed attempts (script: `ralph skill increment-feature-attempts`)
- **`ralph-get-feature-stats`** - Get project stats overview (script: `ralph skill get-feature-stats`)
...
**CRITICAL: ALWAYS use the `ralph skill` commands to update `feature_list.json`.** NEVER edit `feature_list.json` directly with inline `jq` writes, `ConvertTo-Json`, `sed`, or any other method. Direct manipulation has caused data corruption (wiping all features). The scripts handle atomic reads/writes safely. Read-only queries (jq or ConvertFrom-Json) are fine for orientation.
...
> **REMINDER:** You must implement exactly ONE feature this iteration, then exit. Do NOT batch multiple features. Use the `ralph skill` commands for ALL `feature_list.json` writes — never edit it directly.

**DO NOT read entire feature_list.json** - it may be too large. Use targeted queries or skills:

```powershell
# 1. Get project stats (use skill script or inline query)
# Script: ./ralph skill get-feature-stats
$data = Get-Content feature_list.json -Raw | ConvertFrom-Json
$data | Select-Object project, stats, config | ConvertTo-Json -Depth 10

# 2. Find current in-progress feature (if any)
# Script: ./ralph skill get-next-feature
...
```powershell
# Search by ID
$data.features | Where-Object { $_.id -eq 'F042' } | ConvertTo-Json -Depth 10
...
1. Update `feature_list.json` (**MUST use `ralph skill`**):
```powershell
# MANDATORY: Use the script — do NOT edit feature_list.json by hand
./ralph skill update-feature-status --id FXXX --status complete
```
> **NEVER** edit feature_list.json directly — not with jq writes, ConvertTo-Json, sed, or any other method. The companion script recalculates stats and handles atomic writes. Direct edits have caused stats drift and data corruption in past runs.

2. **MANDATORY — Git commit (one feature per commit). DO NOT SKIP THIS STEP:**
```bash
git add .
...
> **WARNING:** If you do not run `git add .` and `git commit` before exiting, your implementation work will be uncommitted and may be lost. This is a hard requirement.
...
**DO NOT commit broken code. DO NOT commit on failure.** Only commit when tests pass (Step 6A).

1. **Search for clues and try to fix it:**
```bash
...
2. Update `feature_list.json` (**MUST use `ralph skill`**):
```powershell
# MANDATORY: Use the script — do NOT edit feature_list.json by hand
./ralph skill increment-feature-attempts --id FXXX --error "error message"
```
> **NEVER** edit feature_list.json directly. The script handles atomic read/write safely.

3. **Update Codebase Patterns** if you discovered anti-patterns:
...
5. **EXIT** — the loop will restart with fresh context and try again. The feature stays `in_progress` and the next iteration will pick it up. After max attempts, it gets marked `blocked` automatically.
...
### DO (HARD RULES):
- ✅ **ALWAYS `git add .` and `git commit` when tests PASS — never skip the commit**
- ✅ **DO NOT commit when tests fail — log the error, exit, and let the loop retry**
- ✅ **Search codebase for patterns before implementing**
- ✅ **ONE feature per iteration — commit on success, then EXIT**
- ✅ **Use `ralph skill` commands for ALL feature_list.json writes** (they recalculate stats atomically)
...
### DON'T:
- ❌ **NEVER commit broken code** — only commit when all tests pass
- ❌ **NEVER edit feature_list.json directly** — always use `ralph skill` commands (direct manipulation causes stats drift and data corruption)
