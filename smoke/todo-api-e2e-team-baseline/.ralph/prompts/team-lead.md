...
- **TaskCompleted hook**: When a task is marked complete, the hook validates that the feature was properly claimed via ``ralph skill claim-feature``. Invalid completions are blocked automatically.
...
- Reviewer updates `feature_list.json` status to `complete` via `ralph skill`
- Report progress
- **Check exit condition above**
...
```
pending
  → in_progress (implementer claims via `ralph skill claim-feature`)
...
- The replacement will pick up `in_progress` features via ``ralph skill claim-feature``
...
- Let teammates self-claim features via ``ralph skill claim-feature``
