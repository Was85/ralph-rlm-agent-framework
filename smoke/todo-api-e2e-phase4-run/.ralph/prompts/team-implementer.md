...
- **`ralph-claim-feature`** - Atomically claim the next available feature (script: `ralph skill claim-feature`) — **USE THIS instead of get-next-feature in team mode**
...
```
1. CLAIM a feature (via `ralph skill claim-feature`)
...
./ralph skill claim-feature --teammate "YOUR_NAME"
```
...
4. **Update feature_list.json** status (**MUST use `ralph skill`**):
```powershell
...
4. **Check attempt count** — if at max attempts (default 5):
   - Mark feature as `blocked`:
     ```powershell
     ./ralph skill update-feature-status --id FXXX --status blocked
     ```
...
### DO:
- Use ``ralph skill claim-feature`` to claim features atomically (prevents double-claiming)
- Use `ralph skill` commands for ALL feature_list.json writes
...
```
[CLAIM]    After `ralph skill claim-feature` returns — log feature ID and your name
...
- **[CLAIM]**: Immediately after ``ralph skill claim-feature`` returns a feature
