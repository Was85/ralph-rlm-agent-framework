...
5. Create `copilot-progress.txt` initialized with Codebase Patterns section
...
The framework provides modular skills in `skills/`. Key skills for initialization:

- **`skills/ralph/get-feature-stats/`** - Get project stats (script: `skills/ralph/get-feature-stats/get-feature-stats.ps1`)
- **`skills/ralph/validate-prd/SKILL.md`** - PRD quality checklist (read before analyzing PRD)
- **`skills/docs-lookup/SKILL.md`** - API verification guidelines (read when encountering unfamiliar APIs)

---

## STEP -1: LOAD INSTRUCTIONS (Auto-Loaded by Copilot)

Domain-specific development instructions are auto-loaded from `.github/instructions/` based on `applyTo:` file patterns. For example:
- `.github/instructions/csharp.instructions.md` applies to `**/*.cs` files

These instructions are automatically active when Copilot works with matching file types. You do not need to manually load them. However, be aware of their conventions when creating features.
...
## STEP 3: CREATE `copilot-progress.txt`
...
```bash
git add feature_list.json copilot-progress.txt
git commit -m "Initialize Ralph-RLM-Framework project

- feature_list.json: N features defined
- copilot-progress.txt: Progress tracking initialized
...
- Verbose notes (save details for copilot-progress.txt)
